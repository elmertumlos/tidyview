...
const TVARRANGE = (() => {
  ...
  function addRow() {
    ...
    div.id    = 'sort-row-' + id;
    div.style.cssText = 'display:grid;grid-template-columns:24px 1fr 1fr 32px;gap:8px;align-items:center;margin-bottom:8px';
    div.innerHTML = `
      <span style="font-size:11px;font-weight:500;color:var(--md-on-surface-variant);text-align:center" id="sort-num-${id}">${rows.length}</span>
      ...
      <button onclick="TVARRANGE.removeRow('${id}')" style="width:28px;height:28px;border-radius:50%;border:none;background:transparent;color:var(--md-on-surface-variant);cursor:pointer;font-size:14px;display:flex;align-items:center;justify-content:center">✕</button>`;
